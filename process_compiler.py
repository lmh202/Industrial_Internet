...
def validate_process_plan(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(populated_plan_fields) > 1:
    ...
    if operations is not None:
        ...
        for index, job in enumerate(jobs, start=1):
            ...
            if action == "produce":
                clean_jobs.append(_validate_produce_job(index, job))
            elif action == "move_to_output":
                clean_jobs.append(_validate_move_job(index, job))
            else:
                raise PlanValidationError(
                    f"Process job {index} has unsupported action: {action!r}"
                )
        clean["jobs"] = clean_jobs
...
def compile_process_plan(process_plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if "operations" in process_plan:
        steps = compile_process_operations(process_plan["operations"], strategy)
    elif "actions" in process_plan:
        operations = _actions_to_operations(process_plan["actions"], strategy)
        steps = compile_process_operations(operations, strategy)
    else:
        operations = _jobs_to_operations(process_plan["jobs"], strategy)
        steps = compile_process_operations(operations, strategy)
...
def compile_process_operations(
    ...
    for operation in clean_operations:
        ...
        if op == "load_base":
            ...
            line_output[operation["line"]] = True
            line_started[operation["line"]] = False
...
def _validate_produce_job(index: int, job: dict[str, Any]) -> dict[str, Any]:
    product = job.get("product")
    if product not in PRODUCT_SPECS:
        raise PlanValidationError(f"Process job {index} has unsupported product: {product!r}")
    quantity = job.get("quantity", 1)
    if isinstance(quantity, bool) or not isinstance(quantity, int) or quantity < 1:
        raise PlanValidationError(f"Process job {index} quantity must be a positive integer.")
    return {"action": "produce", "product": product, "quantity": quantity}


def _validate_move_job(index: int, job: dict[str, Any]) -> dict[str, Any]:
    line = job.get("line")
    if line not in LINE_PARTS:
        raise PlanValidationError(f"Process job {index} has unsupported line: {line!r}")
    part = job.get("part") or DEFAULT_LINE_PART[line]
    if part not in SUPPORTED_PARTS or part not in LINE_PARTS[line]:
        raise PlanValidationError(
            f"Process job {index} cannot move part {part!r} on line {line}."
        )
...
def _validate_action(index: int, action: dict[str, Any]) -> dict[str, Any]:
    ...
    if action_name == "move_to_output":
        return _validate_move_job(index, action)
    raise PlanValidationError(
...
def _validate_operation(index: int, operation: dict[str, Any]) -> dict[str, Any]:
    ...
    if op == "assemble":
        ...
        if supplier not in {"A2", "B2"}:
            raise PlanValidationError(
                f"Process operation {index} supplier must be A2 or B2."
            )
...
def _jobs_to_operations(jobs: list[dict[str, Any]], strategy: str) -> list[dict[str, Any]]:
    ...
    for job in jobs:
        if job["action"] == "produce":
            ...
            line_has_output[line] = True
    return _actions_to_operations(actions, strategy)
...
def _actions_to_operations(
    ...
    operations: list[dict[str, Any]] = []
    if strategy == "parallel_start" and _has_car_and_phone_actions(actions):
        car_operations: list[dict[str, Any]] = []
        phone_operations: list[dict[str, Any]] = []
        for action in actions:
            target = _target_operations_for_action(action, car_operations, phone_operations)
            target.extend(_action_to_operations(action))
        return _interleave_operations(car_operations, phone_operations)
...
def _target_operations_for_action(
    action: dict[str, Any],
    car_operations: list[dict[str, Any]],
    phone_operations: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    line = _line_for_action(action)
    return car_operations if line == "A" else phone_operations
...
def _interleave_operations(
    ...
    operations: list[dict[str, Any]] = []
    max_len = max(len(car_operations), len(phone_operations))
    for index in range(max_len):
        if index < len(car_operations):
            operations.append(car_operations[index])
        if index < len(phone_operations):
            operations.append(phone_operations[index])
    return operations


def _repeated_product_operations(product: str, quantity: int) -> list[dict[str, Any]]:
    operations: list[dict[str, Any]] = []
    for _ in range(quantity):
        operations.extend(_product_operations(product))
    return operations
...
def _has_car_and_phone(jobs: list[dict[str, Any]]) -> bool:
    products = {job["product"] for job in jobs}
    return {"car", "phone"}.issubset(products)
